...
import os
import shutil
from handytool.file import select_folder
...
def spacy_doc2conllu_style_text(doc, filename: str = "", sent_index: int = 0, sent_id_length: int = 3):
    ...
    将spaCy文档转换为CoNLL格式
    - ROOT节点的HEAD列为0
    - 添加元信息：# newdoc id（仅在第一句时添加）、# sent_id、# text
    - 默认sent_id为三位数字
    """
    conllu_lines = []
    for token in doc:
...
def stanza_doc2conllu_style_text(doc, filename: str = None, sent_start_index: int = 0, sent_id_length: int = 3) -> str:
    """
    将 Stanza 的 Document 转换为 CoNLL-U 格式字符串。
    - 第一句添加 # newdoc id
    - 每句添加 # sent_id 和 # text，text 为去空格后的原句
    - 支持起始编号设定（便于句子编号连续）
    """
    lines = []
    for i, sentence in enumerate(doc.sentences):
        ...
        clean_text = sentence.text.replace(" ", "")  # 去除空格还原原句
        lines.append(f"# sent_id = {sent_id}")
...
def conllu_to_latex_standalone(conllu_data):
    ...
    latex_code += "\\setmainfont{Times New Roman} % 替换为你希望使用的字体\n"
...
19  пасля   пасля   ADP IN  _   20  case    20:case _
